...
export default function SignUpScreen() {
  ...
  return (
    <View style={screenStyles.root}>
      ...
      <Text style={screenStyles.muted}>Create an account, then activate a stand</Text>
      <TextInput
        autoCapitalize="none"
      ...
      <Pressable disabled={busy} onPress={() => void onSubmit()} style={screenStyles.button}>
        <Text style={screenStyles.buttonLabel}>{busy ? "Creating…" : "Sign up"}</Text>
      </Pressable>
      <Link href="/(auth)/sign-in" style={{ marginTop: 16 }}>
        <Text style={screenStyles.muted}>Already have an account</Text>
      </Link>
    </View>
  );
}
